...
gps_levels <- names(summary(Training_Data_Complete$gps_height)[1:limitLevels])
gps_height <- factor(Training_Data_Complete$gps_height, levels=c(gps_levels, "Other"))
gps_height[is.na(funder)] <- "Other"
Training_Data_Complete$gps_height <- gps_height

gps_height <- factor(Testing_Data$gps_height, levels=c(gps_levels, "Other"))
gps_height[is.na(funder)] <- "Other"
...
write.csv(submissionOutput, file = "eXtremeGradientBoostigSubmission.csv", row.names = FALSE)
